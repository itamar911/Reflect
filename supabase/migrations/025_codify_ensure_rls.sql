...
--
-- PASTE THE VERBATIM BODY BELOW BEFORE RUNNING. See "Before running".
--
...
-- ── Before running ──
--
-- The BODY block below is a placeholder. Replace it with the verbatim output of
-- query B in supabase/queries/function_bodies.sql, which prints the live
-- function one line per row.
--
-- This matters more here than it did in 022. There, a paraphrase would have
-- been a slightly inaccurate comment on something being deleted. Here it is
-- CREATE OR REPLACE against a live function that turns RLS on — so a
-- reconstruction that is subtly wrong does not sit in a comment, it becomes
-- the behaviour. Paste the real text.
--
-- Two things to preserve exactly when you paste, because they are easy to lose:
--
--   SECURITY DEFINER  — the trigger has to ALTER tables it does not own, so
--                       without this it fails on the first table created by
--                       any role other than the function owner.
--   SET search_path   — a SECURITY DEFINER function without a pinned
--                       search_path can be steered into running someone else's
--                       code. If the live one lacks it, say so rather than
--                       adding it silently; that is a behaviour change and
--                       deserves its own decision.

-- ═══════════════════════════════════════════════════════════════════════════
-- BODY — replace everything between these lines with query B's output.
--
-- What is here is a placeholder describing the live function, NOT the live
-- function. Running it as-is would replace a working trigger with an
...
-- function already had, but a fresh CREATE would grant EXECUTE to PUBLIC by
-- default — so on a new project this line is what stops a SECURITY DEFINER
-- function that runs dynamic ALTER TABLE from being reachable with the anon
-- key. 024 sets the default for future functions; this is belt and braces for
-- this specific one, and costs nothing.
--
-- It does not stop the event trigger firing: an event trigger is invoked by the
-- server while executing DDL, not by a role calling the function, and that path
-- checks no EXECUTE privilege. Verification step 3 proves it.
...
-- 2. The function is still SECURITY DEFINER with a pinned search_path.
--    prosecdef must be true; proconfig must contain a search_path entry.
