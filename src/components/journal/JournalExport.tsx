...
export default function JournalExport({ trades }: { trades: ExportTrade[] }) {
  function handleExport() {
    const headers = ['׳×׳׳¨׳™׳', '׳©׳¢׳”', '׳׳¡׳˜׳¨׳˜׳’׳™׳”', '׳›׳ ׳™׳¡׳”', 'Stop Loss', 'Take Profit', 'R:R', '׳׳¦׳‘ ׳¨׳’׳©׳™', '׳¡׳™׳‘׳× ׳›׳ ׳™׳¡׳”', '׳×׳—׳§׳™׳¨', '׳¡׳˜׳˜׳•׳¡'];
    ...
    const blob = new Blob(['ן»¿' + csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Reflect-trades-${new Date().toISOString().split('T')[0]}.csv`;
