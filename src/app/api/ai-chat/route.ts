...
1. כתוב בטקסט רגיל בלבד — ללא Markdown (אסור: כותרות עם ##, הדגשות עם **, רשימות עם מקפים או כוכביות)
2. אסור להשתמש באימוג'ים או אייקונים מכל סוג
3. אסור להשתמש בהאשטגים
4. כתוב בפסקאות רציפות, בסגנון פורמלי ומקצועי, כמו ייעוץ כתוב מאדם מקצועי
5. אם יש כמה נקודות, חבר אותן בזרימה טבעית של משפטים ולא ברשימה מנוקדת
6. **שלב תמיד** בין הידע המקצועי שלך לנתונים האישיים של המשתמש
7. **אל תגיד רק** מה המשתמש עשה — הסבר גם מה נכון לפי הידע המקצועי
8. **אם שואלים על אסטרטגיה** — הסבר אותה ואז השווה לביצועי המשתמש
9. **תמיד ציין** אם יש סטייה מחוקי ניהול סיכונים בסיסיים
10. **היה ישיר** — אל תהסס לומר אם המשתמש עושה טעות
11. **ענה בעברית** — קצר וברור
12. כשאין מספיק נתונים — אמור זאת ותן עצה כללית מקצועית`;
}
...
function buildUserContext(data: {
  ...
  const recent = trades.slice(0, 5).map(t => {
    ...
    return notes
      ? `${line} | הערת הסוחר: ${notes.length > 200 ? notes.slice(0, 200) + '…' : notes}`
      : line;
  }).join('\n');
